...
\begin{abstract}
  ...
  In this AI driven era where models are largely trained with data collected either via web crawling or offering free or paid online services. Long, text heavy, and hard-to-understand terms and conditions (T\&C) which are the conventional mechanism to inform and receive user consent showing their weaknesses. Several research has concluded that most users do not read and understand T\&C. This is not only because of the complexity of the T\&C but also number of online service one must user to perform their daily activities and they have little or no time to review those. We argue that users have been adequately informed about this data gathering or collection process through T\&C. Therefore, we propose an alternative summarizing T\&C using visual aid that makes hidden practices visible at a glance, highlights unknowns where policies are unclear or missing and emphasize user impact in plain text.
  
  % How many online application a person uses in a day?
  
%  about the mutual benefits for both users and companies. 
  
%  Therefore, we argue that all publicly available AIS can clearly communicate these mutual benefits. In this paper, we introduce a framework called Visual FactSheet for AIS. Its main goal is to visually present one or more attributes of AIS, enabling users to make informed choices before engaging with them.
  
  
%  The development of machine learning models is advancing rapidly and has become a widely recognized concept, largely due to leading technology companies integrating machine learning technique into many of their products. Although these artificially intelligent services (AIS) are gaining popularity at an unprecedented pace among users, there is limited public information about how these machine learning models were built, trained, and what datasets were utilized. Additionally, details on achieving trustworthiness, transparency, and fairness; key principles of AI governance are scarce. Most users of AIS services remain poorly informed what they are agreeing to through terms and conditions prior using services. We argue that users have been adequately informed about the mutual benefits for both users and companies. Therefore, we argue that all publicly available AIS can clearly communicate these mutual benefits. In this paper, we introduce a framework called Visual FactSheet for AIS. Its main goal is to visually present one or more attributes of AIS, enabling users to make informed choices before engaging with them. Furthermore, we propose formal definitions tailored for different FactSheet stakeholders, which we believe will assist in creating effective and meaningful FactSheets.
...
\section{Literature Review}\label{liturea_review}
	...
	In this paper, we introduce multiple FS user-focused categories and provide detailed definitions for each, outlining their respective content. Additionally, we propose a visual documentation framework, i.e., \enquote{Visual FactSheet} (VFS), designed for non-technical users. While Zhang et al. introduced privacy nutrition labels for mobile applications offering a limited subset of information, their approach is not fully applicable to AIS \cite{298912}. Our proposed VFS builds upon and partially resembles Zhang et al.'s contribution but is specifically tailored for AIS.
...
\section{VFS Design and Image Selection Methodology}\label{VFS_Methodology}
...
\subsection{VFS Example}\label{example}
	...
	\begin{figure}[h!]
		\centering
		\scalebox{1}[0.90]{\input{diagram/ChatGPT_VFS.tex}}
...
\section{User Survey}
	...
	% 	Further details about the survey proposal are provided in Appendix XXXX.
	
%	\begin{figure}[h!]
%		\centering
%		\scalebox{0.85}[0.75]{\input{diagram/Survey_Legends.tex}}
%		\caption{Visual FactSheet Legends}
%		\label{fig:VSLegends}
%	\end{figure}
	
%	\begin{figure}[ht]
%		\centering
%		\input{diagram/Survey_Legends_ML.tex}
%		\caption{Visual FactSheet Legends for ML }
%		\label{fig:surveyLegendsML}
%	\end{figure}

	\begin{figure}[h!]
		\centering
		\scalebox{1}[0.90]{\input{diagram/Survey_VFS.tex}}
...
\subsection{Survey Design}
	...
	\begin{figure}[H]
		\centering
		\scalebox{0.85}[0.85] {\includegraphics[page=1, width=\linewidth]{images/pilotsurvey/SurveyQuestion_main.pdf}}
		\caption{Survey Flow}
	...
	%	\begin{figure}[!h]
		%		\centering
		%		\scalebox{0.85}[0.85] {\includegraphics[page=3, width=\linewidth]{images/SurveyQuestion_main.pdf}}
		%		\caption{Survey Flow for Visual}
		%		\label{fig:SuveryFlowVisual}
		%	\end{figure}
	
\section{Participant Sampling and Technology}
	As mentioned earlier, each participant was presented with two versions of the Table of Contents (ToC): a traditional text‑only ToC and its corresponding Visual ToC. A random number between 1 and 2 was generated within the Qualtrics platform to determine the presentation order. This ensured that participants viewed either the text‑only ToC first or the Visual ToC first. Figure \ref{fig:SuveryFlow} illustrates the complete survey flow, which consists of seven sections in which participants provide their responses.

	Because the social media scenario used in this study is entirely fictional, a hypothetical Table of Contents (ToC) was developed for the project and is provided in Appendix XXX. This ToC was created using Microsoft Co-Pilot and is modeled after the structure commonly found in commercial social media platforms such as Facebook. To produce a corresponding Visual ToC, the project adopted the framework outlined in Section \ref{ComponentsVFS}. Figure \ref{fig:Survey_Legends_02} shows a subset of the images used to construct the Visual FactSheet (VFS) equivalent.
...
\subsection{Level of Understanding}
	...
	The survey inquires participants level of understanding after reviewing text only and visual ToC. Data collected through pilot survey shows that participants exhibit higher confidence level after reviewing visual ToC. Figure \ref{fig:confidence} shows that mean confidence for visual ToC is higher than that of text only ToC.
	
	\begin{figure}[!h]
		\centering
		\scalebox{0.75}[0.75] {\includegraphics[page=1, width=\linewidth]{images/pilotsurvey/Survey_Pilot.pdf}}
		%		\includegraphics[ width=\linewidth]{images/Understanding_RatingToC.pdf}
		\caption{Confidence Understanding ToC Agreement}
		\label{fig:confidence}
	\end{figure}

\subsection{NASA Task Load Index}

	The survey measured subjective workload after completing text only and visual ToC following NASA TLX. Pilot survey data shows that participants experienced higher mental demand, shown in Figure \ref{fig:frustation} Question 1, interpreting text only ToC. The mental demand was doubled in comparison to visual ToC. Participants also exhibited higher frustration reading the text only ToC. The frustration level again more than double than that of visual ToC. Figure \ref{fig:frustation} Question 6, represents the data collected during pilot survey.
	
	\begin{figure}[!h]
		\centering
		\includegraphics[width=\linewidth]{images/pilotsurvey/TLX.pdf}
		\caption{Frustration Reading ToC}
		\label{fig:frustation}
	\end{figure}

\subsection{Time Vs Accuracy}

	During the pilot survey, it was discovered, presented in Figure \ref{fig:time} that almost all participants except one spent very less time reviewing text only and visual ToC. However, participants were able to achieve higher accuracy answering survey questions that are directly related to the ToC. For example, when asked about information sharing with their potential employer, participants were able to answer more accurately after reviewing visual ToC. Figure \ref{fig:employer} shows that less participants were unsure and more participants were aware of this fact.
	
	\begin{figure}[!h]
		\centering
		\scalebox{0.75}[0.75] {\includegraphics[page=8, width=\linewidth]{images/pilotsurvey/Survey_Pilot.pdf}}
		\caption{Time Spent Reviewing ToC}
		\label{fig:time}
	\end{figure}
...
\subsection{ToC Preference}

	At the end of the survey, participants were asked about ToC preference, i.e., text only or visual. Data, presented in Figure \ref{fig:rating} shows that majority of participants chose visual over text only ToC.
...
%
%\input{vfs_survey_design/Survey_Invitation.tex}
